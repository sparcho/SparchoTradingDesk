...
_FAMILY_TOKENS = ("the operator", "Fundamental 10G", "family-member-c", "family-member-c", "family-member-d", "HUF", "2P2", "Kite", "SPARCHO")
...
_NAME_MAP = {
    "Account A": "the operator", "Account B": "Fundamental 10G", "Account B-HUF": "Fundamental 10G HUF",
    "Account C": "family-member-c", "Account D": "family-member-d", "Account E": "2P2", "Account F": "Kite",
}
...
# brand and the generic 'HUF'; excludes 'Fundamental 10G' bare-substring checks done carefully below).
_LEAK_TOKENS = ("the operator", "family-member-c", "family-member-c", "family-member-d")
...
_MS_CODE_TO_KEY = {"Y": "family-member-d", "Sp": "the operator", "RKJHUF": "family_huf", "family-member-c": "family-member-c"}
_MS_COST_HDR = {"Y": "Y Cost", "Sp": "Sp Cost", "RKJHUF": "HUF Cost", "family-member-c": "family-member-c Cost"}
...
# -----------------------------------------------------------------------------
# TWO-LAYER DERIVATION (Fundamental 10G 260602): separate PROBABILITY (book-independent)
...
def derive_strategy(cfg, xag_now, book_totals, regime, sbees_now=None):
    ...
    ~Rs.[redacted] (family silver book) -- NOT a separate new-money budget.
    So scope = family_totals: deployed ~= Rs.1.1Cr, dry ~= Rs.3.9Cr. (Supersedes the
    260607 F120 operator-only scoping, which under-counted deployed.)
    """
...
def emit() -> Path:
    ...
    out["silver_strategy"] = {k: _sc.get(k) for k in
        ("schema", "signed", "advisory_only", "fib", "levels_xag",
    ...
    # DERIVED two-layer blocks (Fundamental 10G 260602) -- built from cfg + live state
    ...
    # the operator F109 book ONLY (CLAUDE 8.1.1); Fundamental 10G's units are a separate re-entry
    ...
    # (the 'level-ID-fib' leak, PEOPLE.md / F123b) while leaving sensitive_enc ciphertext byte-exact.
