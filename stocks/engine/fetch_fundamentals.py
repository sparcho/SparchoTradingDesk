...
WHY THIS EXISTS
===============
Fundamental 10G's screener #1 (and any future fundamental-aware screener) needs PEG /
...
consumer (Fundamental 10G #1) actually exercises the gap.
"""
...
COLUMNS = [
    ...
    'peg_3y', 'peg_ttm',
    # Fundamental 8G's 8 (added 2026-04-28; #4 ROIC deferred -- not exposed on screener.in free pages)
    'promoter_pct',                                      # Fundamental 8G #1: Promoter Holding %
    ...
    'opm_pct', 'financing_margin_pct',                   # Fundamental 8G #5: OPM % (NBFC = Financing Margin %)
    'borrowings', 'reserves', 'equity_capital',          # Balance-sheet inputs for D/E
    'de_ratio',                                          # Fundamental 8G #3: computed Borrowings / (Equity + Reserves)
    'icr',                                               # Fundamental 8G #7: computed OpProfit / Interest
    'mcap_to_sales',                                     # Fundamental 8G #8: computed market_cap / sales_or_revenue
...
# =============================================================================
# Section-aware row extraction (P&L, Balance Sheet, Shareholding) -- Fundamental 8G fields
# =============================================================================
...
def parse_fund8g_fields(html: str) -> dict:
    """Extract Fundamental 8G's 6 missing fields."""
...
def compute_fund8g_derived(row: dict) -> dict:
...
def process_ticker(ticker: str) -> dict:
    ...
    # Fundamental 8G fields (added 2026-04-28)
    Fundamental 8G = parse_fund8g_fields(html)
    base_row.update({k: v for k, v in Fundamental 8G.items() if v is not None and k in COLUMNS})
    derived = compute_fund8g_derived(base_row)
