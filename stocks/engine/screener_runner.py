...
# Audit: 00_SYSTEM/AUDITS/260518_Technical-DIV_BULL_drift_audit.md
# =============================================================================
...
# Screener specs
# =============================================================================
def screener_fundamental_10g(ticker, fund, hist_rows, daily_rows):
...
def screener_fundamental_8g(ticker, fund, hist_rows, daily_rows):
...
def screener_technical_div(ticker, fund, hist_rows, daily_rows, hist_vol_rows=None):
...
SCREENER_REGISTRY = {
    ...
    'Technical-DIV': {
        'fn': screener_technical_div,
        'name': 'Technical-DIV -- pure technical: 10d squeeze + volume spikes + RSI divergence',
        'all_gates_required': True,
        'needs_volume': True,
    },
    'Fundamental-8G': {
        'fn': screener_fundamental_8g,
...
# de-cascade proceeds but LevelID-TA's G5 gate is inert. Read in write_outputs() to
...
def write_outputs(name, rows, today):
    ...
    # LevelID-TA's G5 gate was inert this run. Record it so system_doctor flags YELLOW
    # rather than the gate silently returning no fires (F138 / 260701).
    if name == 'LevelID-TA' and globals().get('_LEVELID_G5_DEGRADED', False):
...
def main():
    ...
    # LevelID-TA's engulfing pattern silently returned 'TALIB-NOT-INSTALLED' before this;
    # one of five mechanical gates was dead and the screener didn't tell on itself.
    # Now: if LevelID-TA is in the run list, talib MUST import or we halt loudly.
    _levelid_in_run = (
        args.screener == 'LevelID-TA'
        or (not args.screener and 'LevelID-TA' in SCREENER_REGISTRY)
    )
    if _levelid_in_run:
        try:
            ...
            import sys as _sys
            _levelid_only = (args.screener == 'LevelID-TA')
            print("=" * 72, file=_sys.stderr)
            if _levelid_only:
                # Single-lens intent (`--screener LevelID-TA`): G5 IS the point of this
                # run, so a missing talib is genuinely fatal -> hard-halt (unchanged).
                print("PRE-FLIGHT FAIL: screener_runner.py / LevelID-TA", file=_sys.stderr)
                print("=" * 72, file=_sys.stderr)
                print(f"  TA-Lib not importable: {_ie}", file=_sys.stderr)
                print("  LevelID-TA's CDLENGULFING gate (G5) requires the talib library.", file=_sys.stderr)
            ...
            # watchlists + report -- none of which use talib. LevelID-TA still runs; its
            ...
            print("DEGRADED (non-fatal): screener_runner.py / LevelID-TA G5", file=_sys.stderr)
            print(f"  TA-Lib not importable: {_ie}", file=_sys.stderr)
            print("  LevelID-TA's CDLENGULFING gate (G5) will NOT fire this run; the other", file=_sys.stderr)
...
# =============================================================================
# LevelID-TA chart confluence screener (added 2026-05-02)
# =============================================================================
# Mechanical TA proxy for LevelID's lens. v0 ships 4 gates; >=3 = HIGH-CONF.
# Per /00_SYSTEM/REFERENCES/CONVERSATIONS/LEVELID/LENS_ANALYSIS_v1.md sec 2.1 + 3.1.
...
def _levelid_engulfing_pattern(daily_rows):
    ...
    for i in range(len(vals) - 1, -1, -1):
        if vals[i] != 0:
            ...
            fired = vals[i] > 0  # gate fires TRUE on bullish only (matches LevelID-TA bullish-bias convention)
            return fired, {'engulfing_kind': kind, 'engulfing_value': int(vals[i]), 'engulfing_date': d}
    return None, {'engulfing_kind': None, 'engulfing_value': None, 'engulfing_date': None}



def screener_levelid_ta(ticker, fund, hist_rows, daily_rows):
    """LevelID chart-confluence screener. v0. Bullish-only.
    ...
    Source: /00_SYSTEM/REFERENCES/CONVERSATIONS/LEVELID/LENS_ANALYSIS_v1.md
    """
...
SCREENER_REGISTRY['LevelID-TA'] = {
    'fn': screener_levelid_ta,
    'name': 'LevelID-TA chart confluence -- v0.6: MA stack + RSI momentum + multi-TF align + AOI proximity + CDLENGULFING + HTF Supertrend(W) + HTF Fib proximity (7/7 = HIGHEST-CONF)',
...
# =============================================================================
# Shared TA helpers (260602) — DayTrade lens + LevelID deepening
# =============================================================================
_SR_CACHE = None
...
# proxy for LevelID's confluence method. CVD is a signed-volume PROXY; Orderbook
