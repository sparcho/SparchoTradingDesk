...
# ---------------------------------------------------------------------------
ALL_TICKERS = [
    'OLAELEC',   # +260728 onboarded from the broker drop (HELD)
    'DENTA',   # +260728 onboarded from the broker drop (HELD)
    'ALLCARGO',   # +260901 onboarded from the broker drop (HELD 5,000 since 31-Aug).
    'GRAVITA',    # +260901. It has a hand-verified fib bank and was held until today, but was
                  # never in the universe -- so nothing pulled it each session and its levels
                  # were being judged against an ageing close (test_banking_a_read...).
                  # It had been held for two sessions with no card, no universe entry and no
                  # price, which is also why its 12 chart frames read as an orphan stem.
    # +260905. Operator: AEROFLEX + HINDPETRO "to be onboarded soon"; CASTROLIND arrived as
    # 6 chart frames on 04-Sep. All three were sitting as orphan chart stems -- captured,
    # studied, and invisible to every screen because nothing pulled a price for them.
    'AEROFLEX',     # Aeroflex Industries -- SS corrugated flexible flow solutions, export-led
    'HINDPETRO',    # Hindustan Petroleum -- oil refining & marketing PSU
    'CASTROLIND',   # Castrol India -- automotive & industrial lubricants
    # 10 TA-analysed
    ...
    'AREM','CHAMBLFERT',
    # 260601 operator tracked-universe expansion (+11) — grouping in _inputs/equity_taxonomy.yaml.
    # New names need a historical+fundamentals fetch before the lenses can score them.
    'HINDALCO','HINDCOPPER','VOLTAS',          # L1 minerals / L3 HVAC
...
INTERMARKET_TICKERS = [
    ...
    'BRENT',   # Brent crude (Hormuz / oil gate) — added 2026-05-20 per Phase 4 regime-numerics extension
    'USDJPY',  # USD/JPY (yen-carry pressure proxy for fragile-yields-watch) — added 2026-05-20
]
...
SPECIAL.update({
    ...
    'DXY':    ['DX-Y.NYB'],
    'TNX':    ['^TNX'],
    'BRENT':  ['BZ=F'],     # ICE Brent front-month futures (Yahoo continuous)
    'USDJPY': ['JPY=X'],
})
